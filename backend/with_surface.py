...
def find_head_planes_trial9(points):
    """
    Revised Trial 9:
    - Reduced percentile clipping from 1.0% to 0.1% to preserve valid ends.
    - Lowered bin point threshold from 50 to 25 to account for gaps/sparsity at ends.
    """
    x = points[:, 0]
    ...
    # Relaxed bounds to capture full length (0.1% cut instead of 1.0%)
    x_bounds = np.percentile(x, [0.1, 99.9])
    ...
    for i in range(n_bins):
        ...
        # Lowered threshold to 25 to accept sparser ends
        if np.sum(mask) > 25:
...
# ============================================================================
# PART 2: THE PHYSICS ENGINE & MODEL (From Script 3)
# ============================================================================
